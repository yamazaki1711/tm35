...
def load_daily_progress(cur, daily):
    ...
    for d in daily:
        ...
        val = d["value"]
        crew = val if isinstance(val, (int, float)) else None
        if d["kind"] == "plan":
        ...
        if d.get("data_quality_flag") == "needs_review":
            rec["data_quality_flag"] = "needs_review"
            rec["data_quality_note"] = d.get("data_quality_note")
    ...
    for (work_code, date), rec in merged.items():
        ...
        if work_id is None:
            ...
            continue
        rows.append((
            date, work_id, rec["planned_crew"], rec["actual_crew"], rec["comment"],
    ...
    execute_values(
        cur,
        """
        insert into daily_progress (date, work_id, planned_crew, actual_crew, comment,
