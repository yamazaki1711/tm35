...
# ВЕРСИЯ v2 (29.08.2026) — источник «Сводная Таблица по ТМ 35 от
# 28.08.26.xlsx». Координатор: «Замечания ИКС и Замечания РСК из ветки
# СМР удалить полностью как сущность... Деление работ теперь только по
# двум категориям: основные и вспомогательные». Сам файл это уже
# отражает структурно — раньше было 3 листа (основной график +
# отдельные листы по замечаниям ИКС/РСК), теперь один лист «СМР общий
# ГПР» с двумя секциями по заголовку в колонке A: «Работы по
# устранению выявленных недостатков при строительстве» (= main) и
# «Работы неучтенные в основном графике» (= aux, распознаётся тем же
# AUX_SECTION_TITLE, что и раньше). Секций «замечания ИКС»/«замечания
# РСК» в новом файле нет вообще — конфиги для них удалены, не патчатся
# no-op'ом ради истории (мёртвый код никто не должен принять за живой).
SHEET_CONFIGS = {
    "СМР общий ГПР": {
        "source": "main",
        "fields": {
...
if __name__ == "__main__":
    xlsx = sys.argv[1] if len(sys.argv) > 1 else \
        "/home/oleg/Documents/TM-35/import/smr_280826/Сводная Таблица по ТМ 35 от 28.08.26_fixed.xlsx"
    out = sys.argv[2] if len(sys.argv) > 2 else "/home/oleg/Documents/TM-35/import/smr_280826/output"
    main(xlsx, out)
