...
def clean_join(tokens, word_level=False):
    ...
    for token in tokens[1:]:
        # Координатор, 07.09.2026: дефис внутри ячейки — не перенос строки
        # (тот, что убирается при склейке уже готовых строк в абзац выше),
        # а часть термина ("труба-деталь", "нормативно-технической",
        # "ППУ-изоляция" — проверено по всем 6 срабатываниям этого условия
        # в документе, все шесть оказались составными терминами, ни
        # одного настоящего переноса слова). Дефис сохраняется, убирается
        # только пробел.
