...
def main_check():
    ...
    # --- 11g. Координатор, 25.09.2026 — у нарушения РОВНО один
    # ответственный отдел (миграция 043, rsk_processing.responsible_id),
    # не m2m. С одним полем на нарушение сырая сумма by_responsible[].n
    # по построению не может превысить число нарушений — каждое считается
    # ровно один раз, в ровно одной строке (у одного нарушения не может
    # быть двух отделов одновременно). Тождество: сумма по всем отделам +
    # «без ответственного» = «Активных замечаний», простым sum(), без
    # обхода через distinct(violation_id), который был нужен только пока
    # существовал m2m.
    rsk_sum_by_responsible = sum(r["n"] for r in rsk_dash_stats["by_responsible"])
    check(
        "РСК «по ответственным»: сумма по отделам + «без ответственного» = «Активных замечаний»",
        "сумма по отделам + без", rsk_sum_by_responsible + rsk_dash_stats["no_responsible"],
        "тайл «Активных замечаний»", rsk_dash_stats["tiles"]["total_active"],
    )

    # --- 11h. Координатор, 25.09.2026 — «у нарушения не может быть
    # больше одного ответственного отдела» проверяется буквально по
    # столбцу (не косвенно через сумму 11g, которая не отличила бы «один
    # отдел на всех» от «два отдела, но у разных нарушений» при некоторых
    # совпадениях): единственная возможность нарушить это в новой схеме —
    # прямая правка в БД мимо формы, `responsible_id` — скалярная колонка,
    # структурно не может хранить два значения сразу. Проверка существует
    # на случай будущей регрессии (например, если кто-то вернёт запись в
    # rsk_processing_responsible и по ошибке решит агрегировать из неё
    # вместо колонки).
    rsk_multi_dept = m.query_one("""
        select count(*) as n from (
            select processing_id from rsk_processing_responsible group by processing_id having count(*) > 1
        ) t
    """)["n"]
    check(
